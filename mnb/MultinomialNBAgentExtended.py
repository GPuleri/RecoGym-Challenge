...
def arange_kronecker(features, actions, num_actions):
    ...
    n = actions.size
    # One-hot-encoded taken action
    one_hot_actions = np.zeros_like(features)
    for row in range(one_hot_actions.shape[0]):
        chosen_action = actions[row]
        one_hot_actions[row, chosen_action] = 1
    features = np.hstack((features, one_hot_actions))  # Features are extended with the actions that have been taken. Actions are one-hot-encoded
    ...
    # One-hot-encoded actions added to data

    return sparse.csr_matrix((data, ja, ia), shape=(n, num_features*num_actions), dtype='uint16')  # Kronecker product of the new features is  returned
...
class MultinomialNBAgentModel(Model):
        ...
        if self.batch_size == self.config.online_training_batch:
            ...
            if self.batches_done % self.config.grid_search_interval == 0:
                ...
                self.model = do_grid_search(
                    ...
                    arange_kronecker(self.full_x, self.full_a, self.config.num_products),
                    self.full_y
                )
            else:
                ...
                kronecker_product = arange_kronecker(self.batch_x, self.batch_a, self.config.num_products)
                print(f"Online train shape: {kronecker_product.shape}")
                # print(f"Result of kronecker with added one-hot-encoded action:\n{pd.DataFrame(kronecker_product)}")
                # exit(1)
    ...
    def act(self, observation, features):
        ...
        if random.random() < self.config.epsilon:
            ...
            one_hot_prev_action = np.zeros_like(features)
            one_hot_prev_action[0, self.previous_action] = 1 if self.previous_action else 0
            features = np.hstack((features, one_hot_prev_action))  # The previously taken action is added to features as one-hot-encoded
            matrix = sparse.kron(sparse.eye(features.shape[1] // 2), features, format="csr")
            # actions = np.arange(self.config.num_products)
            # matrix = arange_kronecker(features, actions, self.config.num_products)
...
class MultinomialNBAgentModelBuilder(AbstractFeatureProvider):
    ...
    def build(self):
        ...
        kron = arange_kronecker(offline_features, offline_actions, self.config.num_products)
        print(f"Kroneceker:\n{kron}\nShape: {kron.shape}")
