    ...
    def build(self):
        ...
        class CrossConfirmationMNBAgentModel(Model):
            ...
            def act(self, observation, features):
                ...
                if np.sum(predictions_0) + np.sum(predictions_1) + np.sum(predictions_2) <= self.config.fallback_threshold:
                    action = np.argmax(features)
                elif action_0 == action_1 == action_2:
                    action = action_0
                elif action_0 == action_1 != action_2 or action_0 == action_2 != action_1:
                    if predictions_0[action_0] + predictions_1[action_1] >= predictions_2[action_2]:
                        action = action_0
                    else:
                        action = action_2
                elif action_0 != action_1 == action_2:
                    if predictions_0[action_1] + predictions_1[action_2] >= predictions_2[action_2]:
                        action = action_1
                    else:
                        action = action_0
                else:
