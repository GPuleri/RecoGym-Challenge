...
class AE_LogReg_ModelBuilder(AbstractFeatureProvider):
    ...
    def build(self):
        ...
        print("ok1")
        processed_features = latent_mapper.train(X.todense())
        print("ok2")
...
class LatentMapperSVD(LatentMapper):
    ...
    def train(self, unprocessed_user_views):
        ...
        if self.pre_processor:
            user_views = self.pre_processor.train(unprocessed_user_views)
    ...
    def apply(self, unprocessed_user_view):
        ...
        if self.pre_processor:
            user_view = self.pre_processor.apply(unprocessed_user_view)
