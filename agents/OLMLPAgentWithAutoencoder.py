...
class MLPModelBuilder(AbstractFeatureProvider):
    ...
    def build(self):
        ...
        class MLPModel(Model):
            ...
            def train_online(self, features, action, reward):
                ...
                if self.online_count % self.config.online_training_batch == 0:
                    ...
                    if self.first_time:
                        self.autoencoder = train_lin_encoder(np.array(self.online_data).reshape((self.config.online_training_batch, P)), 2)
            ...
            def act(self, observation, features):
                ...
                if self.online_count < self.config.online_training_batch:
                    ...
                    if random.random() < self.config.epsilon:
                        ...
                        print("Choosing most viewed product")
                        action = np.argmax(features)
                    else:
        ...
        model = MLPClassifier(activation="logistic", solver="adam", hidden_layer_sizes=(P,))
