...
class TestAgent(nn.Module, Agent):
    ...
    def __init__(self, config=Configuration(test_agent_args)):
        ...
        self.id_to_indx = np.empty((1, 0),
                                   dtype=int)  # u_to_indx[session['u']] gives the place at which user nr 'u' arrived
    ...
    def set_model(self):
        if not self.knn_model:
            ...
            print(self.M_organic_scaled_only_clicks.shape[1])
            self.products = np.transpose(self.M_organic_scaled)
            self.knn_model = NearestNeighbors(n_neighbors=self.M_organic_scaled.shape[1],
                                              algorithm='auto').fit(self.products)
    ...
    def pick_best_action(self, distances, nearest_neighbours):
        ...
        for j, neighbour in enumerate(nearest_neighbours):
            ...
            score_items += np.divide(self.user_clicks, self.user_attempts, out=np.zeros_like(self.user_clicks),
                                     where=self.user_attempts != 0)
    ...
    def act(self, observation, reward, done):
        ...
        product = self.products[self.user_organic_scaled.argmax()]

        distances, nearest_neighbours = self.knn_model.kneighbors([product])

        distances = 1 - distances
        for idx, neigh in enumerate(nearest_neighbours):
            distances[idx] *= self.user_organic_scaled[neigh]
        action = distances.argmax()
