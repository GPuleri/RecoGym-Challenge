...
class TestAgent(nn.Module, Agent):
    ...
    def update_information(self, observation, action, reward, done):
        ...
        for session in observation.current_sessions:
            ...
            if user_id != self.last_arrival_id:
                ...
                if self.nr_arrivals-1 != self.last_arrival_indx:
            ...
            if self.new_user and (self.last_arrival_indx % 100 == 0):
                print(self.last_arrival_indx)
            return
    ...
    def update_model(self):
        ...
        self.kNN_model = ci.MultiClusterIndex(self.M_organic_scaled_only_clicks, range(len(self.M_organic_scaled_only_clicks[:,0])))
        return
    ...
    def find_kNN_organic(self):
        ...
        kNN_with_distances=self.kNN_model.search(self.user_organic_scaled, k=self.k, return_distance=True)[0]
        distances=np.zeros((self.k))
        neighbours=np.zeros((self.k), dtype=int)
        for i in range(self.k):
            distances[i]=kNN_with_distances[i][0]
            neighbours[i]=kNN_with_distances[i][1]
    ...
    def pick_best_action(self, distances, nearest_neighbours):
        ...
        score_items+=self.div(self.user_clicks, self.user_attempts)
    ...
    def act(self, observation, reward, done):
        ...
        if observation is not None and len(observation.current_sessions) > 0:
            ...
            if (self.last_arrival_id!=observation.current_sessions[0]['u']):
                ...
                if np.sum(self.user_clicks)>0:
                    ...
                    if self.num_arrivals_since_update%100 == 0:
                        self.update_model()
                        print(self.num_arrivals_since_update)
                        sys.exit()
                self.user_organic=np.zeros((self.config.num_products))
                self.user_clicks=np.zeros_like(self.user_organic)
                self.user_attempts=np.zeros_like(self.user_organic)
        ...
        if new_arrival:
            ...
            self.score=self.pick_best_action(distances, nearest_neighbours)
        #As action we simply pick the one with the highest score
        action = self.score.argmax().item()
